...
/* Demo assets are no longer seeded. The inventory is built by
   AssetDiscoveryService (passive discovery from real captured traffic), so it
   contains exactly the devices seen on the network - nothing fabricated. The
   original demo INSERTs (assets + asset_tags) are kept here, commented, for
   reference only.
...
/* Demo anomalies removed - the anomaly store is now fed only by real signals:
   the DPI rule engine (on pcap upload) and decoy/twin interactions. The original
   demo INSERTs (anomalies + anomaly_tags + anomaly_indicators) are kept here,
